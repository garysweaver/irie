...
module Example
  module Company
    class TestFoobarsController < ActionController::TestCase
     ...
     ## is important that we eager load and run the test of the path value here and
     ## in ::TestFoobarsController (in root, different controller), to ensure that
     ## one helper method doesn't trump another, otherwise applications with
     ## controllers using the same model and controller name in different namespaces
     ...
     #    assert_equal "example_company_#{m}", @controller.send(m)
     #  end
     #end
     ##{foobar_url}, #{foobar_path}, #{foobars_url}, #{foobars_path}, #{edit_foobar_url}, #{edit_foobar_path}, #{new_foobar_url}, #{new_foobar_path}
