    ...
    def output_actionizer_debugging_info
      ...
      all_registered_actionizer_modules = (the_config_class.available_actions.values + the_config_class.available_extensions.values).compact.uniq.collect do |m|
        begin
          ...
          logger.debug("Actionizer config specifies invalid module: #{m}") if Actionizer.debug?
        end
        m
      end
      registered_actionizer_ancestors = the_self_class.ancestors.select {|a|all_registered_actionizer_modules.include?(a)}
      logger.debug("#{the_self_class}.ancestors in #{the_config_desc}.available_actions/#{the_config_desc}.available_extensions:\n\n#{registered_actionizer_ancestors.collect{|s|s.to_s}.join("\n")}\n\n")

      logger.debug("(The following may help you identify methods that are defined in Actionizer includes. Ensure `super if defined?(super)` is done for methods that should support chaining.)\n\n")

      logger.debug("Instance methods in #{the_self_class} and Actionizer-registered ancestors, excluding methods only defined in #{the_self_class}:\n\n")
      ...
      logger.debug("#{instance_method_inheritance_chain.collect{|k,v|"#{k.inspect} = #{v.collect{|a|a.to_s}.join(", ")}"}.sort.join("\n")}\n\n")

    rescue => e
      logger.debug("output_actionizer_debugging_info failed: #{e.message}\n#{e.backtrace.join("\n")}")
    end
  end
end
