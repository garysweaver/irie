  ...
  def initialize
    ...
    unless @__restful_json_initialized
      ...
      if is_activerecord_model?(fully_qualified_restful_json_model_name)
        ...
        #instance_eval "restful_json_model #{fully_qualified_restful_json_model_name.constantize.name}"
        ...
        # This must be run when the class is instantiated, so we'll do the equivalent of having just written this line into the code of the class.
        #instance_eval "restful_json_model #{unqualified_restful_json_model_name.constantize.name}"
      else
